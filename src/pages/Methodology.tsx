    ...
    <>
            ...
            <div className="space-y-8">
              {steps.map((step, i) => (
                <div key={step.number} className="relative flex gap-6 md:gap-10 animate-fade-up" style={{ animationDelay: `${i * 0.1}s` }}>
                  {/* Number bubble */}
                  <div className="relative z-10 w-12 h-12 md:w-16 md:h-16 rounded-full bg-primary/10 border-2 border-primary flex items-center justify-center shrink-0 glow-cyan">
                    <span className="text-primary font-heading font-bold text-sm md:text-base">{step.number}</span>
                  </div>

                  {/* Content */}
                  <div className="glass rounded-xl p-6 md:p-8 flex-1 hover:border-primary/40 transition-all">
                    <div className="flex items-center gap-3 mb-3">
                      <step.icon size={20} className="text-primary" />
                      <h2 className="text-xl font-heading font-semibold text-foreground">{step.title}</h2>
                    </div>
      ...
      <section className="section-padding pt-0">
        <div className="container mx-auto text-center">
          <div className="glass rounded-2xl p-10 md:p-14 glow-purple">
